...
contains
    ...
    do i = buffer_left+period_left+1,buffer_left+2*period_left
        ...
        pidxL(countL:countL+ndims-1) = temp
        countL = countL + ndims
    end do
    ...
    do i = natoms-buffer_right-2*period_right+1,&
       ...
       do j = 1,ndims
        ...
        pidxR(countR:countR+ndims-1) = temp
        countR = countR + ndims
    end do
    ...
    subroutine vel_mat_tdb_mpi(ie,ik_core,E_k,gl,gr,glp,grm,gml,gmr,&
                  G_D_RL,G_D_LR,G_L,G_R,&
                  transl_s,transr_s,transl_ns,transr_ns,&
                  refll_s,reflr_s,refll_ns,reflr_ns,&
                  norb_pc_l,norb_pc_r,&
    ...
    complex(kind=8),intent(in) :: hamlv(:,:,:),hamrv(:,:,:)
    real(kind=8),intent(out) :: transl_s(ne,nk),transr_s(ne,nk)
    real(kind=8),intent(out) :: transl_ns(ne,nk),transr_ns(ne,nk)
    real(kind=8),intent(out) :: refll_s(ne,nk),reflr_s(ne,nk)
    real(kind=8),intent(out) :: refll_ns(ne,nk),reflr_ns(ne,nk)
    ...
    mm=myid+1
    nn = k_start_core(mm)+ik_core-1
    call vel_mat_tdb(ie,mm,nn,E_k(ie,nn),&
        gl(mm,:,:),gr(mm,:,:),glp(mm,:,:),grm(mm,:,:),&
        gml(mm,:,:),gmr(mm,:,:),G_D_RL(mm,:,:),G_D_LR(mm,:,:),&
        G_L(mm,:,:), G_R(mm,:,:), &
        k_grid(nn,:),&
    ...
    end subroutine vel_mat_tdb_mpi
    ...
    use input, only : latvec, positions, recivec,&
                      ...
                      natoms_pc_l,natoms_pc_r,&
                      natoms_uc_l,natoms_uc_r,&
                      pos_pc_l_fc_sc,&
                      pos_pc_r_fc_sc,&
                      mass_uc_l,mass_uc_r

    use fc, only:     force_constant_pc_l,&
                      force_constant_pc_r
    ...
    complex(kind=8) :: F_adv_L_m(nl,nl),lambda_adv_L_m(nl),&
            ...
            V_adv_L_m_half(nl,nl),inv_U_adv_L_m_c(nl,nl),U_adv_L_m_c(nl,nl),&
            lambda_adv_L_m_c(nl),V_adv_L_m_c(nl,nl),V_adv_L_m_half_c(nl,nl)
    ...
    integer(kind=4) :: num_prop_l_adv_c
    ...
    integer(kind=4) :: l_adv_to_propl_c(nl)
    ...
    real(kind=8) :: vpc_unfold_l(125,3),vpc_unfold_r(125,3)
    integer(kind=4) :: kpc_deg_num_l,kpc_deg_num_r
    complex(kind=8) :: test(nl,nl),errors
    real(kind=8)    :: templ(nl),templ1(nl)
    integer(kind=8)  :: tempidx(nl),tempidx1(nl)
...
!    call circular_eigen_axb(gl,Ham(n_buffer_l+1:n_buffer_l+nl,&
!                     n_buffer_l+nl+1:n_buffer_l+2*nl),&
!                    nl,n_bloch_y,n_bloch_x,reci_uc_l(2,:),latvec_uc_l(2,:),&
!                        reci_uc_l(1,:),latvec_uc_l(1,:),lambda_adv_L_m_c,U_adv_L_m_c)
!    tempidx = sorting_index(abs(lambda_adv_L_m_c(:)))
!    tempidx1 = sorting_index(abs(lambda_adv_L_m(:)))
!    write(*,*) abs(lambda_adv_L_m_c(tempidx) - lambda_adv_L_m(tempidx1))
!    write(*,*) maxval(abs(U_adv_L_m(:,tempidx) - abs(U_adv_L_m_c(:,tempidx1))))
!    write(*,*) 'eig'
!    write(*,*) matmul(F_adv_L_m,U_adv_L_m_c(:,1))-lambda_adv_L_m_c(1)*U_adv_L_m_c(:,1)
!    write(*,*) lambda_adv_L_m_c(1)
...
!                stop
    ...
    V_adv_L_m = -matmul(matmul(transpose(dconjg(U_adv_L_m)),&
                gml),U_adv_L_m)*i_imag*a_z_L/2.0d0/sqrt(E)
    V_adv_L_m_c = -matmul(matmul(transpose(dconjg(U_adv_L_m_c)),&
                    gml),U_adv_L_m_c)*i_imag*a_z_L/2.0d0/sqrt(E)
...
!    inv_U_adv_L_m_c = circular_inv(transpose(dconjg(U_adv_L_m_c)),nl,n_bloch_y,n_bloch_x,reci_uc_l(2,:),latvec_uc_l(2,:),&    
!    reci_uc_l(1,:),latvec_uc_l(1,:))
!    write(*,*) maxval(abs(inv_U_adv_L_m-inv_U_adv_L_m_c))
!    stop

    V_adv_L_m_half = sqrtm(V_adv_L_m)
!    do i = 1,nl
!       write(*,*)     V_adv_L_m_c(i,i)
!        write(*,*) U_adv_L_m_c(i,:) 
!    end do

!    V_adv_L_m_half_c = sqrtm(V_adv_L_m_c)
...
!            stop
            ...
            call find_k_pc(E,norb_pc_l,&
                          ...
                          norb_uc_l,nl,&
                          U_adv_L_m(:,i),pos_l,&
                          matmul(ksc,recivec),kpc_unfold_l,&
                          kpc_deg_num_l,vpc_unfold_l,&
    ...
    !write(*,*) dot_product(U_ret_L_m(:,1),U_ret_L_m(:,1))
    !stop
...
!    write(*,*)'maxlambda', maxval(abs(lambda_ret_L_m))
    !stop
    j = 0
    do i = 1, nl
!        write(*,*) abs(lambda_ret_L_m(i))
    ...
    do i = 1,num_prop_l_ret
        U_ret_L_m(:,l_ret_to_propl(i)) = U_ret_L_m(:,l_ret_to_propl(i))/sqrt(dot_product(U_ret_L_m(:,l_ret_to_propl(i)),U_ret_L_m(:,l_ret_to_propl(i))))
    end do
    ...
    !            V_ret_L_m),tf_l)
!    write(*,*) 'here'
!    write(*,*) size(U_ret_L_m,1)
!    eyemat(n)
!    write(*,*) 'omega',sqrt(E)

    inv_U_ret_L_m = inv(U_ret_L_m)
!    inv_U_ret_L_m = lstsq(U_ret_L_m,eyemat_cmplx(size(U_ret_L_m,1)))
!    write(*,*) 'here_ed'
    ...
    do i = 1,nl ! i th column (i th eigenvector)
        if (k_z_L_ret(i).lt.100.0) then
            ...
            k_xyz_L_ret(i,:) = move_in_ws2(matmul(ksc(:) + &
                               ...
                               recivec),reci_uc_l)
!            write(*,*) 'maxloc',maxloc(weightL_ret(i,:),1)
!            write(*,*) 'kxy',k_xyz_L_ret(i,:)
...
!            write(*,*) sqrt(dot_product(k_xyz_L_ret_pc(i,:),k_xyz_L_ret_pc(i,:)))
            vel_xyz_L_ret_pc(i,:) = vpc_unfold_l(1,:)
...
!     end if
    ...
    do i = 1,num_prop_r_ret
        U_ret_R_p(:,r_ret_to_propr(i)) = U_ret_R_p(:,r_ret_to_propr(i))/sqrt(dot_product(U_ret_R_p(:,R_ret_to_propr(i)),U_ret_R_p(:,R_ret_to_propr(i))))
    end do
    end if

    V_ret_R_p = matmul(matmul(transpose(dconjg(U_ret_R_p)),&
                gmr),U_ret_R_p)*i_imag*a_z_R/2.0d0/sqrt(E)
    ...
    do i = 1,nr ! i th column (i th eigenvector)
        if (k_z_R_ret(i)<100.0) then
            ...
            ! Si pure interface, some weird points
            !if ((abs(k_xyz_R_ret(i,3)+ 0.54544) .lt.0.001) .or. (abs(k_xyz_R_ret(i,3)+ 0.13109) .lt.0.001)) then
            !    do j =1,160
            !    if  (abs(lambda_ret_R_p(j)-lambda_ret_R_p(i)).lt.0.001) then
            !        write(*,*) lambda_ret_R_p(j),lambda_ret_R_p(i)
            !        write(*,*)(lambda_ret_R_p(j)-lambda_ret_R_p(i)), log_fbz(lambda_ret_R_p(j),a_z_R) !,reci_uc_r(3,3)
            !    end if
            !    end do
            !end if
            ...
            vpc_unfold_r(:,:) = 0.0d0
            kpc_unfold_r(:,:) = 0.0d0
            !write(*,*) 'find_r_ret',ksc
            !write(*,*) weightR_ret(i,:)

            call find_k_pc(E,norb_pc_r,&
                          ksc_cart,&
                          k_xyz_R_ret(i,:),&
                          norb_uc_r,nr,&
                          U_ret_R_p(:,i),pos_r,&
                          matmul(ksc,recivec),kpc_unfold_r,&
                          kpc_deg_num_r,vpc_unfold_r,&
...
!            write(*,*) 'advR',deg_all(i),deg_all(j)
!            write(*,*) tf_r(deg_all(i),deg_all(j))
!             end if
!            end do
!            end do
...
!     end if
    ...
    do i = 1,num_prop_r_adv
        U_adv_R_p(:,r_adv_to_propr(i)) = U_adv_R_p(:,r_adv_to_propr(i))/sqrt(dot_product(U_adv_R_p(:,R_adv_to_propr(i)),U_adv_R_p(:,R_adv_to_propr(i))))
    end do
    end if
...
! end if
    ...
    Q_L = 0.0d0
    Q_L = (E+(E*eta+eta0)*i_imag)*eyemat(nl) - &
      Ham(n_buffer_l+nl+1:n_buffer_l+2*nl,&
      n_buffer_l+nl+1:n_buffer_l+2*nl) - &
    ...
    Q_R = 0.0d0
    Q_R = (E+(E*eta+eta0)*i_imag)*eyemat(nr) - &
      Ham(norb-n_buffer_r-2*nr+1:&
      ...
      norb-n_buffer_r,norb-n_buffer_r-2*nr+1:&
      norb-n_buffer_r-nr),transpose(dconjg(grm))),&
      Ham(norb-n_buffer_r-2*nr+1:&
      norb-n_buffer_r-nr,norb-n_buffer_r-nr+1:&
      norb-n_buffer_r))
    ...
    trl = i_imag*(matmul(V_ret_R_p_half,trl))*2.0d0*sqrt(E/a_z_R/a_z_L)

    tlr = matmul(inv_U_adv_R_p,V_adv_R_p_half)
    tlr = matmul(G_D_LR,tlr)
    tlr = matmul(inv_U_ret_L_m,tlr)
    tlr = i_imag*matmul(V_ret_L_m_half,tlr)*2.0d0*sqrt(E/a_z_L/a_z_R)

    rrr = matmul(inv_U_adv_R_p,V_adv_R_p_half)
    rrr = matmul(G_R-inv(Q_R),rrr)
    rrr = matmul(inv_U_ret_R_p,rrr)
    rrr = i_imag*(matmul(V_ret_R_p_half,rrr))*2.0d0*sqrt(E/a_z_R/a_z_R) ! ret_R to adv_R

    rll = matmul(inv_U_adv_L_m,V_adv_L_m_half)
    rll = matmul(G_L-inv(Q_L),rll)
    rll = matmul(inv_U_ret_L_m,rll)
    rll = i_imag*matmul(V_ret_L_m_half,rll)*2.0d0*sqrt(E/a_z_L/a_z_L) ! ret_R to adv_L
...
!    if (transl_s .gt. 0.1) then
!    write(*,*)'ts', transl_s
!    write(*,*) 't^+t',real(trace(matmul(tlr,transpose(dconjg(tlr)))))
!    end if
    ...
    do i = 1, nl
        ...
        call write_specular(sqrt(E),nn,tlr,k_xyz_L_ret,k_xyz_R_adv,&
                            k_xyz_L_ret_pc,k_xyz_R_adv_pc,&
                            vel_xyz_L_ret_pc,vel_xyz_R_adv_pc,'tlr') ! rightoleft

        call write_specular(sqrt(E),nn,trl,k_xyz_R_ret,k_xyz_L_adv,&
                             k_xyz_R_ret_pc,k_xyz_L_adv_pc,&
                             vel_xyz_R_ret_pc,vel_xyz_L_adv_pc,'trl') ! lefttoright

        call write_specular(sqrt(E),nn,rrr,k_xyz_R_ret,k_xyz_R_adv,&
    ...
    if (verbosity .eq. 1) then
        ...
        call write_md2md(sqrt(E),nn,trl,k_xyz_R_ret,k_xyz_L_adv,&
                           k_xyz_R_ret_pc,k_xyz_L_adv_pc,&
                           vel_xyz_R_ret_pc,vel_xyz_L_adv_pc,'trl')
        call write_md2md(sqrt(E),nn,rrr,k_xyz_R_ret,k_xyz_R_adv,&
                           k_xyz_R_ret_pc,k_xyz_R_adv_pc,&
                           vel_xyz_R_ret_pc,vel_xyz_R_adv_pc,'rrr')
        call write_md2md(sqrt(E),nn,rll,k_xyz_L_ret,k_xyz_L_adv,&
                           k_xyz_L_ret_pc,k_xyz_L_adv_pc,&
    ...
    subroutine vel_mat_tdb(ie,mm,nn,E,gl,gr,glp,grm,&
                       gml,gmr,G_D_RL,G_D_LR,&
                       G_L,G_R,&
                       ksc,&
                       transl_s,transr_s,&
...
!            stop
    ...
    ! Left retarded propagating from right to left
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    F_ret_L_m = matmul(transpose(conjg(gl)),hamlv)
                   !Ham(n_buffer_l+1:n_buffer_l+nl,&
                   !n_buffer_l+nl+1:n_buffer_l+2*nl))
...
!        write(*,*) abs(lambda_ret_L_m(i))
    ...
    do while (case_no .eq. 1)
        ...
        if (ndeg .gt. 1) then
            ...
            temp_HL = haml + hamlv/lambda_ret_L_m(deg_list(1))+&
                   + transpose(dconjg(hamlv))*lambda_ret_L_m(deg_list(1))
                  !Ham(n_buffer_l+1:n_buffer_l+nl,&
                  !n_buffer_l+1:n_buffer_l+nl)+ &
                  !Ham(n_buffer_l+1:n_buffer_l+nl,&
                  !n_buffer_l+nl+1:n_buffer_l+2*nl)/&
...
!            write(*,*) sqrt(dot_product(k_xyz_L_ret_pc(i,:),k_xyz_L_ret_pc(i,:)))
    ...
    do while (case_no .eq. 1)
        ...
        if (ndeg .gt. 1) then
            ...
            temp_HR = Hamr + hamrv/lambda_ret_R_p(deg_list(1))&
                  + transpose(dconjg(hamrv))*lambda_ret_R_p(deg_list(1))
                  !Ham(norb-n_buffer_r-nr+1:&
                  !norb-n_buffer_r,norb-n_buffer_r-nr+1:&
                  !norb-n_buffer_r)+&
                  !Ham(norb-n_buffer_r-nr+1:&
...
!     end if
    ...
    F_adv_R_p = matmul(transpose(dconjg(gr)),Hamrv)
              !Ham(norb-n_buffer_r-nr+1:&
              !norb-n_buffer_r,norb-n_buffer_r-2*nr+1:&
              !  norb-n_buffer_r-nr))
    ...
    do while (case_no .eq. 1)
        ...
        if (ndeg .gt. 1) then
            ...
            temp_HR = Hamr + Hamrv/lambda_adv_R_p(deg_list(1))&
                   +transpose(dconjg(Hamrv))*lambda_adv_R_p(deg_list(1))
                  !Ham(norb-n_buffer_r-nr+1:&
                  !norb-n_buffer_r,norb-n_buffer_r-nr+1:&
                  !norb-n_buffer_r)+&
                  !Ham(norb-n_buffer_r-nr+1:&
...
! end if
    ...
    Q_L = (E+(E*eta+eta0)*i_imag)*eyemat(nl) - &
          Haml - &
          matmul(matmul(transpose(dconjg(Hamlv)),&
          transpose(dconjg(gl))),& 
          hamlv)- &
          matmul(matmul(Hamlv,glp),&
          transpose(dconjg(Hamlv)))
    ...
    Q_R = (E+(E*eta+eta0)*i_imag)*eyemat(nr) - &
          Hamr - &
          matmul(matmul(transpose(dconjg(Hamrv)),&
          gr),Hamrv)-&
    ...
    do i = 1, nl
        do j = 1, nr
            if ((bz_idx_L_ret(i).eq.bz_idx_R_adv(j)).and.&
                ...
                (bz_idx_R_adv(j).gt.0).and.&
                (bz_idx_L_ret(i).gt.0)) then
                !write(*,*)bz_idx_L_ret(i),bz_idx_R_adv(j)
                !write(*,*) weightL_ret(i,:)
                !write(*,*) weightR_adv(j,:)
                !write(*,*) k_z_L_ret(i),k_z_R_adv(j)
                !write(*,*) mm,nn
                !write(*,*) abs(tlr(i,j))**2
