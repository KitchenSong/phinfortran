...
!    write(*,*) maxval(abs(dyn_pc - transpose(dconjg(dyn_pc))))
!    stop
    call eigen_sorting(dyn_pc,eigs_c,evs)
    eigs = abs(eigs_c)
    dyn_pc_p = 0.0d0
    dyn_pc_m = 0.0d0
    dHdk = 0.0d0
    ...
    vxyz = 0.0d0
    do i =1,norb_pc
        if (abs(Eq-sqrt(abs(eigs(i)))).lt.2.0d-1)then
            vxyz = vels(i,:)
            i_deg_pc = 1
            branch_idx_pc = i
    ...
    real(kind=8) :: kpc(3),kpc_ws(3),weight
        ...
        do i1 = -l,l
            do i2 = -l,l
                do i3 = -l,l
                    ...
                    kpc = move_in_ws3(kpc,recivec_pc)
                    ...
                    if (ibz.ne.0) then
                      ...
                      !        natoms_uc,natoms_pc,mass_uc,&
                      !        idx_temp)
                        call gen_dyn_pc_qe_k(kpc,dyn_pc,mass_uc)
...
!        call eigenH(dyn,eig,vec)
    ...
    complex(kind=8) :: dyn(:,:)
    complex(kind=8)    :: vec(size(dyn,1),size(dyn,2))
    complex(kind=8)    :: eig(size(dyn,1))
    real(kind=8)    :: eig_r(size(dyn,1))
    real(kind=8)    :: kpoint(3),cell_reci(3,3)
    real(kind=8),allocatable   :: kpath_pt(:,:)
    real(kind=8),allocatable   :: kpath(:,:) 
    integer(kind=4),allocatable :: nkpath(:)
    ...
    do itemp = 1,npt_total  
        ...
        eig_r = sqrt(abs(eig))
        if (nline .eq. 1) then
            write(31,'(6F10.5)') eig_r
        else
            do jtemp = 1,nline-1
                write(31,'(6F10.5)') eig_r((jtemp-1)*6+1:jtemp*6)
            end do
            write(31,'(6F10.5)') eig_r((nline-1)*6+1:size(eig_r))
        end if
    end do
    close(unit=31)
...
!    kpath_pt(3,:) = (/0.507998467008678d0,       0.507998467008678d0,       0.380828638909260d0/)
    ...
    do itemp = 1,npt_total  
        kpoint = kpath(itemp,:) !matmul(kpath(itemp,:),cell_reci) 
        nscx = 1
        nscy = 1
        nscz = 1
        ...
        eig_r = sqrt(abs(eig))
        if (nline .eq. 1) then
            write(11,'(6F10.5)') eig_r
        else
            do jtemp = 1,nline-1
                write(11,'(6F10.5)') eig_r((jtemp-1)*6+1:jtemp*6)
            end do
            write(11,'(6F10.5)') eig_r((nline-1)*6+1:size(eig_r))
        end if
    end do
    close(unit=11)
    end subroutine
    ...
    subroutine find_neighbors(neighborlist,nnbrs,pos_all,cell_all,natom_all,cell_uc,idx_all,pos_uc)
    ...
    real(kind=8) :: cell_all(3,3),cell_uc(3,3)
    real(kind=8) :: pos_all(:,:)
    real(kind=8) :: mindist,rdiff(3),rdd
    real(kind=8) :: n_uc_ijk(3),R_uc_xyz(3)
    integer(kind=4) :: idx_all(:)
    real(kind=8) :: pos_uc(:,:)
    ...
    do i = 1,natom_all
        do j = 1,natom_all
            ...
            do l =-1,1
                do m =-1,1
                    do n =-1,1
                        R_uc_xyz(:) = matmul((/dble(l),dble(m),dble(n)/),&
                        cell_all)+pos_all(j,1:3)-pos_all(i,1:3)&
                        -pos_uc(idx_all(j),1:3) + pos_uc(idx_all(i),1:3) 
                        ...
                        if (rdd.lt. mindist) then
                            mindist = rdd
                            n_uc_ijk = matmul(R_uc_xyz,inv_real(cell_uc))
                        end if
                    end do
                end do
            end do
            if (update_nbr.eq.1) then
                ...
                if ((abs(n_uc_ijk(1)).lt.1.5d0).and.(abs(n_uc_ijk(2)).lt.1.5d0).and.(abs(n_uc_ijk(3)).lt.1.5d0)) then
                    nnbrs(i) = nnbrs(i)+1
                    neighborlist(i,nnbrs(i)) = j
                end if 
            end if
