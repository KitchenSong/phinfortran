...
contains
    ...
    ! number of orbitals in left buffer region
    do i = 1, buffer_left
        n_buffer_l = n_buffer_l + atom_orb_num(i)
    end do

    ! number of orbitals in right buffer region
    do i = natoms-buffer_left+1, natoms
        n_buffer_r = n_buffer_r + atom_orb_num(i)
    end do

    ! number of orbitals in periodic cell of left lead
    do i = buffer_left+1, buffer_left + period_left
        nl = nl + atom_orb_num(i)
    end do
    
    ! number of orbitals in periodic cell of right lead
    do i = natoms-buffer_right-period_right+1, &
           natoms-buffer_right
        nr = nr + atom_orb_num(i)
    end do
    ...
    reci_uc_l = 2.0*pi*transpose(inv_real(latvec_uc_l))
    ...
    reci_uc_r = 2.0*pi*transpose(inv_real(latvec_uc_r))
    ...
    Ham_l_i_o(:,:) = transpose(dconjg(Ham_l_o_i(:,:)))
    ...
    norb-n_buffer_r)
    Ham_r_i_o(:,:) = transpose(dconjg(Ham_r_o_i(:,:))) 
    ...
    mm = myid + 1
    nn = k_start_core(mm)+ik_core-1
    sdos_l(ie,nn) =  real(aimag(trace(gl_adv(mm,:,:)))/pi)
    sdos_r(ie,nn) = -real(aimag(trace(gr_ret(mm,:,:)))/pi)
